package org.s3s3l.yggdrasil.utils.log.base;
...
public class LogHelper implements ILogger {
	...
	private static final IJsonHelper json = JacksonUtils.defaultHelper;
	private final LogLevel logLevel;

	private boolean isLogRemote = false;
	...
	public void setLogRemote(boolean isLogRemote) {
	...
	private void doLog(LogLevel level, String content, Object... objects) {
	...
	protected void log(LogLevel level, String argName, Object arg) {
		...
		sb.append(stack.getClassName()).append(SEPARATOR).append(stack.getMethodName()).append(SEPARATOR)
				.append(argName).append(SUB_SEPARATOR).append(json.toJsonString(arg)).append(BREAKLINE);
		String content = sb.toString();
		doLog(level, content, arg);
	}
	...
	protected void log(LogLevel level, String msg, String argName, Object arg) {
		...
		StackTraceElement stack = getStackTraceElement();
		sb.append(stack.getClassName()).append(SEPARATOR).append(stack.getMethodName()).append(SEPARATOR)
				.append("Message").append(SUB_SEPARATOR).append(msg).append(BREAKLINE).append(argName)
				.append(SUB_SEPARATOR).append(json.toJsonString(arg)).append(BREAKLINE);
		String content = sb.toString();
		doLog(level, content, arg);
	}
	...
	protected void log(LogLevel level, String msg) {
		StringBuilder sb = new StringBuilder();
		StackTraceElement stack = getStackTraceElement();
		sb.append(stack.getClassName()).append(SEPARATOR).append(stack.getMethodName()).append(SEPARATOR).append(msg)
				.append(BREAKLINE);
		String content = sb.toString();
		doLog(level, content);
	}
	...
	protected void log(LogLevel level, String[] argNames, Object... args) {
		...
		for (int i = 0; i < argNames.length; i++) {
			...
			sb.append(argNames[i]).append(SUB_SEPARATOR).append(json.toJsonString(args[i])).append(BREAKLINE);
		}
		String content = sb.toString();
		doLog(level, content, args);
	}
	 ...
	 * @param argNames 参数名称
	 * @param args     参数内容11对应
	 * @return ClassName MethodName Message : msg; argName1 : arg1 argName2 : arg2;
	 *         ...
	 */
	protected void log(LogLevel level, String msg, String[] argNames, Object... args) {
		...
		StackTraceElement stack = getStackTraceElement();
		sb.append(stack.getClassName()).append(SEPARATOR).append(stack.getMethodName()).append(SEPARATOR)
				.append("Message").append(SUB_SEPARATOR).append(msg).append(BREAKLINE);
		for (int i = 0; i < argNames.length; i++) {
			if (args.length <= i) {
				break;
			}
	...
	protected void log(LogLevel level, Throwable e) {
		...
		sb.append(e.getClass().getName()).append(SUB_SEPARATOR).append(e.getMessage()).append(BREAKLINE)
				.append(sw.toString()).append(BREAKLINE);
		String content = sb.toString();
		doLog(level, content);
	}
	...
	protected void log(LogLevel level, Throwable e, String tag) {
		StringBuilder sb = new StringBuilder();
		StackTraceElement stack = getStackTraceElement();
		sb.append(stack.getClassName()).append(SEPARATOR).append(stack.getMethodName()).append(SEPARATOR).append("tag")
				.append(SUB_SEPARATOR).append(tag).append(BREAKLINE);
